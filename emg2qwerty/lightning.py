...
class RNNCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        # After per-band MLP, we flatten bands -> num_features
        num_features = self.NUM_BANDS * mlp_features[-1]

        # Import here or at top: from emg2qwerty.modules import RNNEncoder
        self.model = nn.Sequential(
            SpectrogramNorm(channels=self.NUM_BANDS * self.ELECTRODE_CHANNELS),
            MultiBandRotationInvariantMLP(
                in_features=in_features,
            ...
            nn.Flatten(start_dim=2),  # (T, N, bands=2, F) -> (T, N, 2*F)

            # Swap THIS part: TDSConvEncoder -> RNNEncoder
            RNNEncoder(
                num_features=num_features,
                hidden_size=hidden_size,
                num_layers=num_layers,
    ...
    def _step(
        ...
        # NOTE: Unlike TDSConvEncoder, RNNEncoder does NOT change time length.
        # So emissions.shape[0] == inputs.shape[0], and T_diff should be 0.
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
